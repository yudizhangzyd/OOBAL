...
Original file is located at
    https://colab.research.google.com/drive/1Xxa8-jLunHl3Jie7iAawFxFWKcOoCjmC
"""
...
def find_data(xn_train, new_y, yn_train, xn_test, yn_test, learning_rate, 
    ...
    for epoch in range(num_epochs):
        for minibatch in minibatches:
            (items, labels) = minibatch
            ## here the shape only suitable for sequence data (each obs is 1d)
            ## if 2D, then the code needs to be changed from minibatch
            items = items.reshape((items.shape[0], 1, items.shape[1]))
            items = torch.from_numpy(items).float().to(device)
            labels = torch.from_numpy(labels).long().to(device)
...
#             y_ec.append([0, 1])
    ...
    draw = choice(range(xn_test.shape[0]), B2, replace=False, p = new_y)
    ...
    complement = np.setdiff1d(range(xn_test.shape[0]), draw)
    yn_test = yn_test[complement]
    xn_test = xn_test[complement, :, :]
    
    return (x, y, xn_test, yn_test)
...
rand, rand_tm = rand_cnn(x_data, y_data, device, iteration = 50)

with open("rand_res.pickle", "wb") as rand_res:
    pickle.dump(rand, rand_res)

with open("rand_time.pickle", "wb") as rand_time:
    pickle.dump(rand_tm, rand_time)

al, al_tm = ac(x_data, y_data, device, iteration = 50)

with open("al_res.pickle", "wb") as al_res:
    pickle.dump(al, al_res)

with open("al_time.pickle", "wb") as al_time:
    pickle.dump(al_tm, al_time)
...
def bt_al(x_data, y_data, device, m, test_size = 0.33, seed = 0, prop = 0.5, B0_p = 0.5, num_epochs = 500,
          num_classes = 2, learning_rate = 0.001, mini_batch_size = 64, bt_samples = 50,
    ...
    for i in range(iteration):
      ...
      for i in range(m + 1):
          ...
          xn_train, yn_train, xn_test, yn_test = find_data(xn_train, new_y, yn_train, xn_test, yn_test, learning_rate, 
                            num_epochs, mini_batch_size, device, num_classes, seed, b)
          if (i != m):
            yn_train = yn_train.detach().cpu().numpy()
            xn_train = xn_train.detach().cpu().numpy()
            xn_test = xn_test.detach().cpu().numpy()
            yn_test = yn_test.detach().cpu().numpy()
            if (xn_train.shape[1] == 1):
                xn_train = np.squeeze(xn_train, axis=1) 
                xn_test = np.squeeze(xn_test, axis=1) 
...
bst_al, bst_tm = bt_al(x_data, y_data, device, m = 3, test_size = 0.33, seed = 0, prop = 0.5, B0_p = 0.5, num_epochs = 300,
          num_classes = 2, learning_rate = 0.001, mini_batch_size = 64, bt_samples = 50,
          num_epochs2 = 800, batch_size2 = 128, iteration = 30, verbose = False)

with open("bst_res300.pickle", "wb") as bst_res:
    pickle.dump(bst_al, bst_res)

with open("bst_time300.pickle", "wb") as bst_time:
    pickle.dump(bst_tm, bst_time)

from google.colab import files
files.download("bst_time300.pickle")
files.download("bst_res300.pickle")

f = open("bst_time.pickle", 'rb')
bst_time = pickle.load(f)
f.close()  
f = open("bst_res.pickle", 'rb')
bst_al = pickle.load(f)
f.close()

f = open("rand_res.pickle", 'rb')
rand_res = pickle.load(f)
f.close()  
f = open("rand_time.pickle", 'rb')
rand_time = pickle.load(f)
f.close()
...
bp = ax.boxplot(res) 
ax.set_ylabel('Time')
plt.xticks([1, 2], ['OOB AL', 'Bootstrapped AL'])
# show plot 
plt.savefig('comparison_t.jpg', bbox_inches='tight', dpi=350)
plt.show()

res = [rand_res, al, bst_al] 
...
bp = ax.boxplot(res) 
ax.set_ylabel('Loss')
plt.xticks([1, 2, 3], ['Random', 'OOB AL', 'Bootstrapped AL'])
# show plot 
